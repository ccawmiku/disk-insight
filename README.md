...
- 60 个等间隔修改时间 Heatmap、容量历史、Treemap 和子目录占用排行
...
镜像默认以 `10001:10001` 运行。Synology、群晖或其他 NAS 可以在 `.env` 中设置宿主机用户：
...
扫描目录必须实际映射到容器内，并允许该用户读取和遍历；SQLite 状态目录必须允许该用户写入。`user` 只改变进程身份，不会自动创建 `/data`：
...
services:
  disk-insight:
    ...
    volumes:
      - /volume1/media:/data:ro
      - /volume1/docker/disk-insight:/var/lib/disk-insight
```

部署后可以验证：

```sh
docker exec disk-insight sh -c \
  'id; ls -ld /data /var/lib/disk-insight; test -r /data && echo data-readable; test -w /var/lib/disk-insight && echo state-writable'
```
...
services:
  disk-insight:
    image: ghcr.io/ccawmiku/disk-insight:v1.1.0
    environment:
...
| `GOMEMLIMIT` | `600MiB` | Go 运行时软内存目标；不包含全部 SQLite/native 内存 |
| `GOGC` | `75` | 更积极的垃圾回收，降低大型扫描峰值 |

应用没有内置登录系统，设计目标是可信本机或局域网。不要直接暴露到公网；如需远程访问，请在前面配置带身份认证和 TLS 的反向代理。

## 扫描与性能

- 扫描器单路、流式遍历，不把完整文件列表或每文件 identity Map 载入内存。
- 文件和目录以小批次写入 SQLite WAL，降低频繁同步写入。
- 硬链接实际空间去重和图表统计由 SQLite 流式/聚合查询完成。
- SQLite 使用两个连接、16 MiB 页面缓存目标、文件临时区且不启用 mmap。
...
- 图表查询使用固定数量的统计桶和 32 项有界缓存；目录树按需加载并虚拟化渲染。
- 页面只轮询扫描进度；未变化的响应不会重绘图表或覆盖未保存设置。
...
- **Visualization:** linked size distribution, cumulative curves, Waffle, equal-interval modification heatmap, Treemap, ranking, and history
